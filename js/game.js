...
const ASSETS = (() => {
  const files = [
    'assets/bgm_maze_loop.mp3',
    'assets/bgm_paint_loop.mp3',
    'assets/bgm_runner_loop.mp3',
    'assets/bgm_shape_loop.mp3',
    'assets/bgm.mp3',
    'assets/bomb-frute.mp3',
    'assets/bomb.mp3',
    'assets/bomb.wav',
    'assets/boomb.mp3',
    'assets/sfx_clear.mp3',
    'assets/sfx_done.mp3',
    'assets/sfx_eraser.mp3',
    'assets/sfx_hit.mp3',
    'assets/sfx_jump.mp3',
    'assets/sfx_paint_stroke.mp3',
    'assets/sfx_point.mp3',
    'assets/sfx_pop_small.mp3',
    'assets/sfx_popup.mp3',
    'assets/sfx_segment_complete.mp3',
    'assets/sfx_shape_complete.mp3',
    'assets/sfx_wrong.mp3',
    'assets/slice-fruit.mp3',
    'assets/slice-frute.mp3',
    'assets/slice.mp3',
    'assets/slice.wav'
  ];
  ...
  const pick = (candidates) => {
    ...
    const sfx = {
    point: pick(['assets/sfx_point.mp3']),
    popup: null,
    segment_complete: pick(['assets/sfx_segment_complete.mp3']),
    shape_complete: pick(['assets/sfx_shape_complete.mp3']),
    wrong: pick(['assets/sfx_wrong.mp3']),
    pop_small: null,
    clear: pick(['assets/sfx_clear.mp3']),
    done: pick(['assets/sfx_done.mp3']),
    eraser: pick(['assets/sfx_eraser.mp3']),
    hit: pick(['assets/sfx_hit.mp3']),
    jump: pick(['assets/sfx_jump.mp3']),
    paint_stroke: pick(['assets/sfx_paint_stroke.mp3']),
  ...
  const bgmVariants = {
    paint: pick(['assets/bgm_paint_loop.mp3']),
    shape: pick(['assets/bgm_shape_loop.mp3']),
    runner: pick(['assets/bgm_runner_loop.mp3']),
    maze: pick(['assets/bgm_maze_loop.mp3']),
    default: pick(['assets/bgm.mp3'])
  };
...
if (!window.__handNinja._simpleAudio) {
  const fallbackMap = {
    bgm: (ASSETS && ASSETS.bgm) ? ASSETS.bgm : 'assets/bgm.mp3',
    // prefer canonical semantic SFX detected above, fall back to common filenames
    slice: (ASSETS && ASSETS.sfx && ASSETS.sfx.slice) ? ASSETS.sfx.slice : (ASSETS && ASSETS.slice) ? ASSETS.slice : 'assets/slice-frute.mp3',
    bomb: (ASSETS && ASSETS.sfx && ASSETS.sfx.bomb) ? ASSETS.sfx.bomb : (ASSETS && ASSETS.bomb) ? ASSETS.bomb : 'assets/boomb.mp3',
    point: (ASSETS && ASSETS.sfx && ASSETS.sfx.point) ? ASSETS.sfx.point : 'assets/sfx_point.mp3',
    popup: (ASSETS && ASSETS.sfx && ASSETS.sfx.popup) ? ASSETS.sfx.popup : null,
    segment_complete: (ASSETS && ASSETS.sfx && ASSETS.sfx.segment_complete) ? ASSETS.sfx.segment_complete : 'assets/sfx_segment_complete.mp3',
    shape_complete: (ASSETS && ASSETS.sfx && ASSETS.sfx.shape_complete) ? ASSETS.sfx.shape_complete : 'assets/sfx_shape_complete.mp3',
    wrong: (ASSETS && ASSETS.sfx && ASSETS.sfx.wrong) ? ASSETS.sfx.wrong : 'assets/sfx_wrong.mp3',
    pop_small: (ASSETS && ASSETS.sfx && ASSETS.sfx.pop_small) ? ASSETS.sfx.pop_small : null,
    clear: (ASSETS && ASSETS.sfx && ASSETS.sfx.clear) ? ASSETS.sfx.clear : 'assets/sfx_clear.mp3',
    done: (ASSETS && ASSETS.sfx && ASSETS.sfx.done) ? ASSETS.sfx.done : 'assets/sfx_done.mp3',
    eraser: (ASSETS && ASSETS.sfx && ASSETS.sfx.eraser) ? ASSETS.sfx.eraser : 'assets/sfx_eraser.mp3',
    paint_stroke: (ASSETS && ASSETS.sfx && ASSETS.sfx.paint_stroke) ? ASSETS.sfx.paint_stroke : 'assets/sfx_paint_stroke.mp3',
    jump: (ASSETS && ASSETS.sfx && ASSETS.sfx.jump) ? ASSETS.sfx.jump : 'assets/sfx_jump.mp3',
    hit: (ASSETS && ASSETS.sfx && ASSETS.sfx.hit) ? ASSETS.sfx.hit : 'assets/sfx_hit.mp3'
  };
...
function playSound(name) {
  try {
    ...
    if (name === 'bgm') {
      ...
      const bgmUrl = ASSETS && ASSETS.bgm ? ASSETS.bgm : 'assets/bgm.mp3';
...
function playLayeredAudio(soundName, baseVolume = 1.0) {
  ...
  for (let i = 0; i < layerCount; i++) {
    ...
    setTimeout(() => {
      // Try to find the asset with 'assets/' prefix
      const assetPath = layerSound.startsWith('assets/') ? layerSound : `assets/${layerSound}`;
...
class FatResponsiveAudioEngine {
  ...
  playSingleLayerSound(soundFile, options = {}) {
    ...
    const assetPath = soundFile.startsWith('assets/') ? soundFile : `assets/${soundFile}`;
...
async function startGame() {
  ...
  try { stopMenuPreviewLoop(); } catch(e){}
    ...
    if (currentGameId === 'ninja-fruit') {
      ...
      ASSETS.slice = ASSETS.slice || 'assets/slice-frute.mp3' || 'assets/slice-fruit.mp3';
      ASSETS.bomb = ASSETS.bomb || 'assets/boomb.mp3';
      ASSETS.sfx = Object.assign({}, ASSETS.sfx, {
        slice: ASSETS.sfx.slice || 'assets/slice-frute.mp3',
        bomb: ASSETS.sfx.bomb || 'assets/boomb.mp3',
        point: ASSETS.sfx.point || 'assets/sfx_point.mp3',
        popup: ASSETS.sfx.popup || null
      });
    } else if (currentGameId === 'paint-air') {
      // paint-air: dedicated paint bgm and SFX from root assets
      ASSETS.bgm = ASSETS.bgmVariants && ASSETS.bgmVariants.paint ? ASSETS.bgmVariants.paint : 'assets/bgm_paint_loop.mp3';
      ASSETS.slice = null;
      ASSETS.bomb = null;
      ASSETS.sfx = Object.assign({}, ASSETS.sfx, {
        clear: ASSETS.sfx.clear || 'assets/sfx_clear.mp3',
        done: ASSETS.sfx.done || 'assets/sfx_done.mp3',
        pop_small: ASSETS.sfx.pop_small || null,
        paint_stroke: ASSETS.sfx.paint_stroke || 'assets/sfx_paint_stroke.mp3'
      });
    } else if (currentGameId === 'shape-trace') {
      // shape-trace: dedicated bgm + shape SFX
      ASSETS.bgm = ASSETS.bgmVariants && ASSETS.bgmVariants.shape ? ASSETS.bgmVariants.shape : 'assets/bgm_shape_loop.mp3';
      ASSETS.slice = null;
      ASSETS.bomb = null;
      ASSETS.sfx = Object.assign({}, ASSETS.sfx, {
        point: ASSETS.sfx.point || 'assets/sfx_point.mp3',
        segment_complete: ASSETS.sfx.segment_complete || 'assets/sfx_segment_complete.mp3',
        shape_complete: ASSETS.sfx.shape_complete || 'assets/sfx_shape_complete.mp3',
        popup: ASSETS.sfx.popup || null,
        wrong: ASSETS.sfx.wrong || 'assets/sfx_wrong.mp3'
      });
    } else if (currentGameId === 'runner-control') {
      // runner-control: runner bgm and gameplay SFX
      ASSETS.bgm = ASSETS.bgmVariants && ASSETS.bgmVariants.runner ? ASSETS.bgmVariants.runner : 'assets/bgm_runner_loop.mp3';
      ASSETS.slice = null;
      ASSETS.bomb = null;
      ASSETS.sfx = Object.assign({}, ASSETS.sfx, {
        point: ASSETS.sfx.point || 'assets/sfx_point.mp3',
        bomb: ASSETS.sfx.bomb || 'assets/sfx_hit.mp3',
        popup: ASSETS.sfx.popup || null,
        jump: ASSETS.sfx.jump || 'assets/sfx_jump.mp3'
      });
    } else if (currentGameId === 'maze-mini') {
      // maze-mini: maze bgm and feedback SFX
      ASSETS.bgm = ASSETS.bgmVariants && ASSETS.bgmVariants.maze ? ASSETS.bgmVariants.maze : 'assets/bgm_maze_loop.mp3';
      ASSETS.slice = null;
      ASSETS.bomb = null;
      ASSETS.sfx = Object.assign({}, ASSETS.sfx, {
        point: ASSETS.sfx.point || 'assets/sfx_point.mp3',
      ...
      ASSETS.bgm = ASSETS.bgm || `assets/bgm.mp3`;
      ASSETS.slice = ASSETS.slice || `assets/slice.mp3`;
      ASSETS.bomb = ASSETS.bomb || `assets/bomb.mp3`;
      ASSETS.sfx = Object.assign({}, ASSETS.sfx, {
        point: ASSETS.sfx.point || 'assets/sfx_point.mp3',
        popup: ASSETS.sfx.popup || null
      });
...
function handleMusicToggle(enabled) {
  // Check room state first. If in a room and NOT admin, do NOT apply the local toggle
  // to avoid transient audio changes — immediately revert the checkbox to the server value.
  const roomsState = (window.ROOMS_UI && window.ROOMS_UI.state) ? window.ROOMS_UI.state : null;
  if (roomsState && roomsState.room) {
    // Non-admin: ignore local change and restore authoritative value
    if (!roomsState.isAdmin) {
      const serverVal = (roomsState.room && typeof roomsState.room.musicEnabled === 'boolean')
        ? roomsState.room.musicEnabled
        : !!musicEnabled;
      try {
        // Do not call setMusicEnabled here to prevent transient local audio start/stop.
        syncMusicCheckboxes(!!serverVal);
        try { flashNotice('Music controlled by room admin'); } catch(e){}
      } catch (e) {
        console.warn('Failed to revert music checkbox for non-admin', e);
      }
      return;
    }

    // Admin: apply locally and persist to server
    try {
      setMusicEnabled(!!enabled);
      syncMusicCheckboxes(!!enabled);
    } catch (e) {
      console.warn('Failed to apply music toggle as admin', e);
    }

    try {
      if (typeof NET !== 'undefined' && typeof NET.setRoomOptions === 'function') {
        NET.setRoomOptions({ musicEnabled: !!enabled }, (res) => {
          if (!res || !res.ok) console.warn('setRoomOptions failed', res);
        });
      }
    } catch (e) {
      console.warn('Failed to setRoomOptions for musicEnabled', e);
    }
    return;
  }

  // Not in a room: local toggle applies normally
  setMusicEnabled(enabled);
  syncMusicCheckboxes(enabled);
}
...
function updateGameBGM(newGameId, { force = false } = {}) {
  ...
  if (newGameId === 'ninja-fruit') {
    newBgm = 'assets/bgm.mp3';
  } else if (newGameId === 'paint-air') {
    newBgm = 'assets/bgm_paint_loop.mp3';
  } else if (newGameId === 'shape-trace') {
    newBgm = 'assets/bgm_shape_loop.mp3';
  } else if (newGameId === 'runner-control') {
    newBgm = 'assets/bgm_runner_loop.mp3';
  } else if (newGameId === 'maze-mini') {
    newBgm = 'assets/bgm_maze_loop.mp3';
  } else {
    newBgm = 'assets/bgm.mp3'; // fallback
  }
...
if (gameSel) {
  gameSel.addEventListener('change', (e) => {
    ...
    // Always update local currentGameId so UI reflects selection
    currentGameId = newGameId;

    // If in a room, honor server-authoritative flow
    if (roomsState && roomsState.room) {
      // Non-admins cannot change room game — revert selector to server value
      if (!roomsState.isAdmin) {
        try {
          const serverGame = (roomsState.room && roomsState.room.game) ? roomsState.room.game : currentGameId;
          if (gameSel) gameSel.value = serverGame;
          flashNotice('Game selection controlled by room admin');
        } catch (e) { console.warn('Failed to revert game selector for non-admin', e); }
        return;
      }

      // Admin: persist choice to server so server broadcasts room_update -> clients will update
      try {
        if (typeof NET !== 'undefined' && typeof NET.setRoomOptions === 'function') {
          NET.setRoomOptions({ game: newGameId }, (res) => {
            if (!res || !res.ok) {
              console.warn('setRoomOptions failed', res);
              flashNotice('Failed to apply room game selection');
              return;
            }
            // Apply BGM immediately for admin UX; server will also broadcast authoritative room_update
            try { updateGameBGM(newGameId, { force: true }); } catch (err) { console.warn('updateGameBGM after setRoomOptions failed', err); }
          });
        } else {
          // No NET available: fall back to local update
          updateGameBGM(newGameId, { force: true });
        }
      } catch (e) {
        console.warn('Admin gameSelect change failed', e);
        updateGameBGM(newGameId, { force: true });
      }
    } else {
      // Not in a room — local change is fine
  ...
  try {
    ...
    NET.on('room_update', (data) => {
      try {
        ...
        // Apply authoritative musicEnabled flag from server so all clients stay in sync
        try {
          if (data && typeof data.musicEnabled === 'boolean') {
            try { setMusicEnabled(!!data.musicEnabled); syncMusicCheckboxes(!!data.musicEnabled); } catch(err) { console.warn('Failed to apply musicEnabled from room_update', err); }
          }
        } catch (e) {
          console.warn('musicEnabled handling failed on room_update', e);
        }
